...
const Metrics = () => {
  ...
  useEffect(() => {
    ...
    const url = userId
      ? `https://fitwithmpt.pythonanywhere.com/api/metrics/${date}/?__user_id=${userId}`
      : `https://fitwithmpt.pythonanywhere.com/api/metrics/${date}/`;
  ...
  const handleSubmit = async () => {
    try {
      ...
      const url1 = userId
        ? `https://fitwithmpt.pythonanywhere.com/api/metrics/?__user_id=${userId}`
        : `https://fitwithmpt.pythonanywhere.com/api/metrics/`;
      await axios.post(
        url1,
        { ...formData, date },
        { headers: { Authorization: `Bearer ${token}` } } // Include the token in the headers
