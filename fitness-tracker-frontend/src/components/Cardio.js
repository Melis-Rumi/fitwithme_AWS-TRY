...
const Cardio = () => {
  ...
  useEffect(() => {
    ...
    const url = userId
  ? `https://fitwithmpt.pythonanywhere.com/api/cardio/${date}/?__user_id=${userId}`
  : `https://fitwithmpt.pythonanywhere.com/api/cardio/${date}/`;
    axios
      .get(url, {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const url1 = userId
      ? `https://fitwithmpt.pythonanywhere.com/api/cardio/?__user_id=${userId}`
      : `https://fitwithmpt.pythonanywhere.com/api/cardio/`;
      await axios.post(
        url1,
        { ...formData, date },
        { headers: { Authorization: `Bearer ${token}` } } // Include the token in the headers
