...
const Diet = () => {
  ...
  useEffect(() => {
    ...
    const url = userId
  ? `https://16.171.79.44/api/diet/${date}/?__user_id=${userId}`
  : `https://16.171.79.44/api/diet/${date}/`;
  ...
  const handleSearchChange = async (e) => {
    ...
    if (query.length >= 2) {
      try {
        const response = await axios.get(`https://16.171.79.44/api/nutrients/search/?search=${query}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.post(
        'https://16.171.79.44/api/diet/',
        { ...formData, date },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      // Fetch updated diet records from the backend
      const response = await axios.get(`https://16.171.79.44/api/diet/${date}/`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setDietRecords(response.data);
  ...
  const handleDelete = async (index) => {
    try {
      ...
      await axios.delete(`https://16.171.79.44/api/diet/${recordToDelete.id}/`, {
        headers: { Authorization: `Bearer ${token}` },
      });
