...
const Diet = () => {
  ...
  useEffect(() => {
    ...
    const url = userId
  ? `https://fitwithmpt.pythonanywhere.com/api/diet/${date}/?__user_id=${userId}`
  : `https://fitwithmpt.pythonanywhere.com/api/diet/${date}/`;
  ...
  const handleSearchChange = async (e) => {
    ...
    if (query.length >= 2) {
      try {
        const response = await axios.get(`https://fitwithmpt.pythonanywhere.com/api/nutrients/search/?search=${query}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.post(
        'https://fitwithmpt.pythonanywhere.com/api/diet/',
        { ...formData, date },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      // Fetch updated diet records from the backend
      const response = await axios.get(`https://fitwithmpt.pythonanywhere.com/api/diet/${date}/`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setDietRecords(response.data);
